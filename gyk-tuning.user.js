...
// @resource      gyk-tuning-css  https://github.com/juzraai/gyk-tuning/raw/dev/gyk-tuning.css
...
function topic(doc) {
	$('#main').fadeOut('fast', function() {
		...
		var q_box = $("table.kerdes tr.kerdes_fejlec ~ tr", doc).get(0);
		var q = $("h1",q_box).text();
		var d =	$("td:has(h1)",q_box).clone().children().remove().end().text();
